...
func (p *advancedFrameProcessor) ReadFrameWithFlags(reader io.Reader) ([]byte, uint32, error) {
	...
	if p.frameTimeout > 0 {
		if conn, ok := reader.(interface {
			...
			// 设置读取帧头的超时
			if err := conn.SetReadDeadline(time.Now().Add(p.frameTimeout)); err != nil {
				return nil, 0, err
			}
			// 确保函数返回时重置超时
	...
	// 如果帧很大，重新设置超时
	if p.frameTimeout > 0 && frameHeader.Size > DefaultLargeChunkSize {
		if conn, ok := reader.(interface {
			SetReadDeadline(t time.Time) error
		}); ok {
			// 根据数据大小调整超时
			timeout := p.frameTimeout
			additionalTime := time.Duration(frameHeader.Size/(1024*1024)) * 10 * time.Second
			if additionalTime > 5*time.Minute { // 限制最大附加时间
				additionalTime = 5 * time.Minute
			}
